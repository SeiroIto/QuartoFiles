...
* ファイルのversion controlプログラム  
   * ファイル: A &rarr; A+B &rarr; A+B-C &rarr; A+...+Z、A+Bに戻りたい! A+B-Cて何だったっけ?  
   * Version control: 全改変記録を保持していつでも閲覧、回帰(とある版に戻ること)が可能  
:::
----
Gitとは  
...
* ファイルのdistributed version controlプログラム  
   ...
   * Bさん: B2を共有レポジトリにpushする
   * Aさん: B2を共有レポジトリから自分のPCにpullする=B2をベースにして作業する  
   * 差分=A2-A1部分、B2-A1部分: ファイルの違う箇所・機能を作業分担するのが原則  
...
* ファイルのdistributed version controlプログラム  
   ...
   * 最新ファイルはサーバではなく、[各自のPCに保存]{style="color:red;"}  
      ...
      * 所有者は参加者全員  
         * ブロックチェイン(分散型台帳システムdistributed ledger system)に似てます  
...
::: {.incremental}
* Gitを扱うwebサービス  
* レポジトリをwebに作成、分散版管理しながら共同作業ができる  
* Webページも公開できる  
