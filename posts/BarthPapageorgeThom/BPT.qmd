...
* Gene-wealth gradient&rarr;0 when stock ownership is added
   * EA score $\propto$ risk torelance, longevity, stock ownership, precision in macroeconomic risk perception
...
1. Into genotyping: Older, more educated, more females, wealthier [(Table S1)](BarthPapgergeThome_SNPWealth_JPE2020Appendix.pdf#page=5) 
   * Education $\propto$ genotyping &rArr; individuals with low EA scores have higher than average education  &rArr; attenuates EA score $\propto$ education gradient 
1. Of using retired households: More precise wealth but smaller sample size/more selection [(Table 3)](BarthPapgergeThome_SNPWealth_JPE2020.pdf#page=17) 
   ...
   * Age band of retired HHs &uarr; than main sample &rArr; q4-q1 differences (more selection) &uarr; &rArr; EA score $\propto$ education gradient &uarr; (Panel A vs. B)
   * $+$non-retired HHs &rArr; q4-q1 differences &darr; (less selection) &rArr; EA score $\propto$ education gradient &darr; (Panel A vs. C)
...
[(p.19)](BarthPapgergeThome_SNPWealth_JPE2020.pdf#page=19)
...
::: {.column width="70%"}
:::{.fragment}
EA score $\propto$ wealth gradient: .246 (raw)&rarr;.070 (+edu)&rarr;.047 (+labor income)
:::

:::{.fragment}
Gradient: "ability" + school quality &larr; income measure error
:::
:::
...
* EA score $\propto$ wealth gradient &rarr;0 if we add stock ownership  
:::
...
::: {.column width="60%"}
* EA score $\propto$ wealth, conditional on DB pension participation
* EA score $\propto$ wealth&uarr; if not participating in DB pension
:::
...
* Previous studies: EA scores $\propto$ education
* This study: EA scores $\propto$ wealth
* EA scores $\propto$ probabilistic thinking &rArr; human capital accumulation heterogeneity  
