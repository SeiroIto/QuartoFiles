...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_92 AnnualInc_PE_92 AnnualInc_SE_92 AnnualIncTot_92
...
keep HHID PN AnnualProfSE AnnualSalSE AnnualInc_SE_94 HoursPerWeekSE WeeksPerYearSE
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_94 AnnualInc_PE_94 AnnualInc_SE_94 AnnualIncTot_94
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_96 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_98 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_00 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_02 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_04 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_06 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_08 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_10 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
keep HHID PN *SUBHH *FINR Working *Retired* HomeMaker WorkForPay SelfEmp_12 AnnualInc_PE_ AnnualInc_SE_ AnnualIncTot_
...
foreach Var in Working Retired HomeMaker WorkForPay SelfEmp AnnualInc_PE AnnualInc_SE AnnualIncTot {
	foreach YRInd in 92 94 96 98 00 02 04 06 08 10 12  {
...
keep HHID PN SUBHH YEAR Working Retired HomeMaker WorkForPay SelfEmp AnnualInc_PE AnnualInc_SE AnnualIncTot


save "$CleanData\HRSEmpIncPanel.dta", replace
...
*********************************************
* DROP FLAG: Keep only years 2010 and Before
*********************************************

drop if Year>2010	
	
	
* First, get a number of Self-Emploment Variables:
*    EverSelfEmp:  Person-level variable indicating ever having a 1 
*    AnySelfEmpHH:  Household (SUBHH) - Year observation for any SelfEmployee
*    EverHHSelfEMP:  Person-level variable indicating whether they were
*                   ever in a household (SUBHH) with any self-employee	
		
bys HHID PN: egen EverSelfEmp=max(SelfEmp)
bys HHID SUBHH YEAR: egen AnySelfEmpHH=max(SelfEmp)
bys HHID PN: egen EverHHSelfEmp=max(AnySelfEmpHH)
	
	
* Now, get NumWorkHH, indicating the number of individuals indicating that
*      they work for pay:	
bys HHID SUBHH YEAR: egen NumWorkHH=total(WorkForPay), missing 	
...
gen RealAnnualIncTotal=AnnualIncTot/CPI	
	...
	* First, generate AIncTotSUBHH, which gives a household total 
	* real income at the HHID-SUBHH-YEAR level:
	gen Temp=.
	replace Temp=RealAnnualIncTotal if Retired==0 & WorkForPay==1
	bys HHID SUBHH YEAR: egen AIncTotSUBHH=total(Temp), missing
	drop Temp	
	
	* In what follows, we will want to calculate some variables at the HHID-SUBHH
	* level, and it will be useful to keep track of only 1 observation for
	* each year for each HHID-SUBHH, so create a counter to keep track:
	
	bys HHID SUBHH     : gen HHID_SUBHHCounter=_n
	bys HHID SUBHH YEAR: gen HHID_SUBHH_YrCounter=_n	
	
	* Now, get the number of years in which we have non-missing income data
	* for this SUBHH:
	gen Temp=0
	replace Temp=1 if AIncTotSUBHH~=. & HHID_SUBHH_YrCounter==1
	bys HHID SUBHH: egen NumHHIDSUBHHIncObs=total(Temp)
	drop Temp
	
	* Now, get the Average Real Income for all years for which we have 
	* non-missing income data for the household:
	gen Temp=.
	replace Temp=AIncTotSUBHH if HHID_SUBHH_YrCounter==1
	bys HHID SUBHH: egen AvgIncSUBHH=mean(Temp)
	drop Temp
	gen logAvgIncSUBHH=log(AvgIncSUBHH)
	
	* Now, get the Maximum for all years for which we have non-missing income data
	gen Temp=.
	replace Temp=AIncTotSUBHH if HHID_SUBHH_YrCounter==1
	bys HHID SUBHH: egen MaxIncSUBHH=max(Temp)
	drop Temp
	gen logMaxIncSUBHH=log(MaxIncSUBHH)	
	
	keep if HHID_SUBHHCounter==1
	
	keep HHID SUBHH logMaxIncSUBHH logAvgIncSUBHH NumHHIDSUBHHIncObs
	
	rename logMaxIncSUBHH           logMaxEmpInc
	rename logAvgIncSUBHH           logAvgEmpInc
	rename NumHHIDSUBHHIncObs       NumHHIDSUBHHEmpIncObs

	
		
save "$CleanData/HRSEmpIncMeasures.dta", replace

/* Get Summary Retirement Variables */

use "$CleanData/HRSEmpInc92.dta"

foreach yr in 94 96 98 00 02 04 06 08 10 12 {
	merge 1:1 HHID PN using "$CleanData/HRSEmpInc`yr'.dta", gen(retmrg`yr')
	replace YearRetired_`yr'=. if YearRetired_`yr'>3000
	replace YearRetired_`yr'=. if YearRetired_`yr'==0
}
...
replace YearRetired   =YearRetired_92 if Retired_92==1
replace YearRetiredMax=YearRetired_92 if Retired_92==1

foreach yr in 94 96 98 00 02 04 06 08 10 12{
	replace YearRetired   =YearRetired_`yr' if Retired_`yr'==1 & YearRetired_`yr'~=.
	replace YearRetiredMax=YearRetired_`yr' if (Retired_`yr'==1 & YearRetired_`yr'~=. & YearRetiredMax==.) | (Retired_`yr'==1 & YearRetired_`yr'~=. & YearRetiredMax~=. & (YearRetired_`yr'>YearRetiredMax))
}

keep HHID PN YearRetired YearRetiredMax 

save "$CleanData/HRS_YearRetired.dta", replace 
