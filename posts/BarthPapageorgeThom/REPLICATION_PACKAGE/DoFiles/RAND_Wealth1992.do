...
gen IRAContVal_`year' = inlist(h`wave'afira, 1, 0)
gen IRAImpVal_`year' = inlist(h`wave'afira, 1, 0, 2) 
...
*********************************************************************/
clear all
infile using "$HRSSurveys92/h92sta/HOUSEHLD.dct" , using("$HRSSurveys92/h92da/HOUSEHLD.da")
local year "1992"
sort HHID ASUBHH
gen SUBHH=ASUBHH
destring HHID, gen(numHHID)
destring SUBHH, gen(numSUBHH)
gen h1hhid=numHHID*10+numSUBHH
...
replace RecInher_1_`year'  = 0 if V5301==5 | (V5301==1 & V5304==1) | (V5301==1 & V5305==1)
replace RecInher_1_`year'  = 1 if V5303==1

gen YearInher_1_`year'     = V5306
replace YearInher_1_`year' = . if inlist(V5306, 9998, 9999)

gen MonthInher_1_`year'    = .
gen double InherAmt_1_`year' = V5307
replace InherAmt_1_`year'  = . if V5307 > 1000000 /* this is from actual tabulation in codebook since
                                                  there are values like 9999985 for some reason */
/*                  14.  R'S PARENTS
                    15.  BROTHER
                    17.  SISTER
                    19.  OTHER RELATIVE
...
gen InherRecWho_1_`year'     = V5308

gen RecInher_2_`year'      = .
replace RecInher_2_`year'  = 0 if V5301==5 | (V5301==1 & V5310==1) | (V5301==1 & V5311==1)
replace RecInher_2_`year'  = 1 if V5309==1

gen YearInher_2_`year'     = V5312
replace YearInher_2_`year' = . if inlist(V5312, 9998, 9999)

gen MonthInher_2_`year'    = .
gen InherAmt_2_`year'      = V5313
replace InherAmt_2_`year'  = . if V5313 > 1000000
gen InherRecWho_2_`year'     = V5314

gen RecInher_3_`year'      = .
replace RecInher_3_`year'  = 0 if V5301==5 | (V5301==1 & V5316==1) | (V5301==1 & V5317==1)
replace RecInher_3_`year'  = 1 if V5315==1

gen YearInher_3_`year'     = V5318
replace YearInher_3_`year' = . if inlist(V5318, 9998, 9999)

gen MonthInher_3_`year'    = .
gen InherAmt_3_`year'      = V5319
replace InherAmt_3_`year'  = . if V5319 > 1000000
gen InherRecWho_3_`year'     = V5320


/*********************************************************************************************
	MERGE WITH RAND DATA
*********************************************************************************************/
keep h1hhid HHID SUBHH *Inher*
save "$CleanData/IRAStocks`year'.dta", replace

isid HHID
merge 1:m h1hhid using "$CleanData/temp1`year'.dta"
