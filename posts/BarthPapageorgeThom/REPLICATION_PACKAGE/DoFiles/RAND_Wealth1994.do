...
*********************************************************************/
clear all
infile using "$HRSSurveys94/h94sta/W2K.dct" , using("$HRSSurveys94/h94da/W2K.da")
local year "1994"
sort HHID CSUBHH
gen SUBHH=CSUBHH
destring HHID, gen(numHHID)
destring SUBHH, gen(numSUBHH)
gen h2hhid=numHHID*10+numSUBHH

/********************************************************************************************************
                                 Inheritance Code	
********************************************************************************************************/
gen RecInher_1_`year'      =.
replace RecInher_1_`year'  =0 if W5565==5 | (W5566<=97 & W5566>=1)
replace RecInher_1_`year'  =1 if W5566==3

gen YearInher_1_`year'     =W5568
replace YearInher_1_`year' =. if W5568==8 | W5568==9

gen MonthInher_1_`year'    =W5567
replace MonthInher_1_`year'=. if W5567==98 | W5567==99

gen InherAmt_1_`year'      =W5569
replace InherAmt_1_`year'  =. if W5569==9999998 | W5569==9999999
/*                  14.  R'S PARENTS
                    15.  BROTHER
                    17.  SISTER
                    19.  OTHER RELATIVE
...
gen InherRecWho_1_`year'     =W5571

gen RecInher_2_`year'      =.
replace RecInher_2_`year'  =0 if W5572==5 | (W5573<=97 & W5573>=1)
replace RecInher_2_`year'  =1 if W5573==3

gen YearInher_2_`year'     =W5575
replace YearInher_2_`year' =. if W5575==8 | W5575==9

gen MonthInher_2_`year'    =W5574
replace MonthInher_2_`year'=. if W5574==98 | W5574==99

gen InherAmt_2_`year'      =W5576
replace InherAmt_2_`year'  =. if W5576==9999998 | W5576==9999999

gen InherRecWho_2_`year'     =W5578

gen RecInher_3_`year'      =.
replace RecInher_3_`year'  =0 if W5579==5 | (W5580<=97 & W5580>=1)
replace RecInher_3_`year'  =1 if W5580==3

gen YearInher_3_`year'     =W5582
replace YearInher_3_`year' =. if W5582==8 | W5582==9

gen MonthInher_3_`year'    =W5581
replace MonthInher_3_`year'=. if W5581==98 | W5581==99

gen InherAmt_3_`year'      =W5583
replace InherAmt_3_`year'  =. if W5583==9999998 | W5583==9999999

gen InherRecWho_3_`year'     =W5585

/*********************************************************************************************
	MERGE WITH RAND DATA
*********************************************************************************************/
keep h2hhid HHID SUBHH *Inher*
isid h2hhid

save "$CleanData/IRAStocks`year'.dta", replace
merge 1:m h2hhid using "$CleanData/temp1`year'.dta"
