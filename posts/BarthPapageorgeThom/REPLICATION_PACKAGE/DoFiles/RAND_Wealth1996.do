...
*****************************************/
gen SecMortgVal_`year'       =.						
gen HasSecMortg_`year'       =.
...
*********************************************************************/
clear all
infile using "$HRSSurveys96/h96sta/H96J_H.dct" , using("$HRSSurveys96/h96da/H96J_H.da")
local year "1996"
sort HHID ESUBHH
gen SUBHH=ESUBHH
destring HHID, gen(numHHID)
destring SUBHH, gen(numSUBHH)
gen h3hhid=numHHID*10+numSUBHH

/********************************************************************************************************
                                 Inheritance Code	
********************************************************************************************************/
gen RecInher_1_`year'      =.
replace RecInher_1_`year'  =0 if E4748==5 | (E4749<=7 & E4749>=1)
replace RecInher_1_`year'  =1 if E4749==3

gen YearInher_1_`year'     =E4751
replace YearInher_1_`year' =. if E4751==9998 | E4751==9999

gen MonthInher_1_`year'    =E4750
replace MonthInher_1_`year'=. if E4750==98 | E4750==99

gen InherAmt_1_`year'      =E4752
replace InherAmt_1_`year'  =. if E4752==999998 | E4752==999999
/*                  14.  R'S PARENTS
                    15.  BROTHER
                    17.  SISTER
                    19.  OTHER RELATIVE
...
gen InherRecWho_1_`year'     =E4754

gen RecInher_2_`year'      =.
replace RecInher_2_`year'  =0 if E4755==5 | (E4756<=7 & E4756>=1)
replace RecInher_2_`year'  =1 if E4756==3

gen YearInher_2_`year'     =E4758
replace YearInher_2_`year' =. if E4758==8 | E4758==9

gen MonthInher_2_`year'    =E4757
replace MonthInher_2_`year'=. if E4757==98 | E4757==99

gen InherAmt_2_`year'      =E4759
replace InherAmt_2_`year'  =. if E4759==999998 | E4759==999999

gen InherRecWho_2_`year'     =E4761

gen RecInher_3_`year'      =.
replace RecInher_3_`year'  =0 if E4762==5 | (E4763<=7 & E4763>=1)
replace RecInher_3_`year'  =1 if E4763==3

gen YearInher_3_`year'     =E4765
replace YearInher_3_`year' =. if E4765==9998 | E4765==9999

gen MonthInher_3_`year'    =E4764
replace MonthInher_3_`year'=. if E4764==98 | E4764==99

gen InherAmt_3_`year'      =E4766
replace InherAmt_3_`year'  =. if E4766==999998 | E4766==999998

gen InherRecWho_3_`year'     =E4768

/*********************************************************************************************
	MERGE WITH RAND DATA
*********************************************************************************************/
keep h3hhid HHID SUBHH *Inher*
isid h3hhid
save "$CleanData/IRAStocks`year'.dta", replace

merge 1:m h3hhid using "$CleanData/temp1`year'.dta"
ren _m StockInheritance

gen FinWealth_NoImputeNoH_`year'  =FinWealth_NoImpute_`year'  -(HouseVal_`year'-MortgVal_`year'-HmLnVal_`year') ///
    if HouseContVal_`year' == 1 & MortgContVal_`year' == 1 & HmLnContVal_`year' == 1
gen FinWealth_WithImputeNoH_`year'=FinWealth_WithImpute_`year'-(HouseVal_`year'-MortgVal_`year'-HmLnVal_`year') ///
    if HouseImpVal_`year' == 1 & MortgImpVal_`year' == 1 & HmLnImpVal_`year' == 1

sort hhidpn
drop if PN==""
gen WealthInStocks_NoImpute_`year'   =.
gen WealthInStocks_WithImpute_`year' =.
gen RiskyWeight1_NoImpute_`year'     =.
gen RiskyWeight1_WithImpute_`year'   =.
gen RiskyWeight1_NoImputeNoH_`year'  =.
gen RiskyWeight1_WithImputeNoH_`year'=.
gen RiskyWeight1_NoImputeBus_`year'  =.
gen RiskyWeight1_WithImputeBus_`year'=.
