...
*********************************************************************/
clear all
infile using "$HRSSurveys98/h98sta/H98J_H.dct" , using("$HRSSurveys98/h98da/H98J_H.da")
local year "1998"
sort HHID FSUBHH
gen SUBHH=FSUBHH
destring HHID, gen(numHHID)
destring SUBHH, gen(numSUBHH)
gen h4hhid=numHHID*10+numSUBHH

/************************************
	IRA 1 STOCK ALLOCATIONS
************************************/
gen IRA1PercStocks=0
replace IRA1PercStocks=100 if F4907==1
replace IRA1PercStocks=0   if F4907==2
replace IRA1PercStocks=50  if F4907==3
replace IRA1PercStocks=.   if F4907==7 | F4907==8 | F4907==9

/************************************
	IRA 2 STOCK ALLOCATIONS
************************************/
gen IRA2PercStocks=0
replace IRA2PercStocks=100 if F4928==1
replace IRA2PercStocks=0   if F4928==2
replace IRA2PercStocks=50  if F4928==3
replace IRA2PercStocks=.   if F4928==7 | F4928==8 | F4928==9

/********************************************************************************************************
                                 Inheritance Code	
********************************************************************************************************/
gen RecInher_1_`year'      =.
replace RecInher_1_`year'  =0 if F5508==5 | (F5509<=7 & F5509>=1)
replace RecInher_1_`year'  =1 if F5509==3

gen YearInher_1_`year'     =F5511
replace YearInher_1_`year' =. if F5511==9998 | F5511==9999

gen MonthInher_1_`year'    =F5510
replace MonthInher_1_`year'=. if F5510==98 | F5510==99

gen InherAmt_1_`year'      =F5512
replace InherAmt_1_`year'  =. if F5512==9999998 | F5512==9999999
/*                  14.  R'S PARENTS
                    15.  BROTHER
                    17.  SISTER
                    19.  OTHER RELATIVE
...
gen InherRecWho_1_`year'     =F5514

gen RecInher_2_`year'      =.
replace RecInher_2_`year'  =0 if F5515==5 | (F5516<=7 & F5516>=1)
replace RecInher_2_`year'  =1 if F5516==3

gen YearInher_2_`year'     =F5518
replace YearInher_2_`year' =. if F5518==9998 | F5518==9999

gen MonthInher_2_`year'    =F5517
replace MonthInher_2_`year'=. if F5517==98 | F5517==99

gen InherAmt_2_`year'      =F5519
replace InherAmt_2_`year'  =. if F5519==9999998 | F5519==9999999

gen InherRecWho_2_`year'     =F5521

gen RecInher_3_`year'      =.
replace RecInher_3_`year'  =0 if F5522==5 | (F5523<=7 & F5523>=1)
replace RecInher_3_`year'  =1 if F5523==3

gen YearInher_3_`year'     =F5525
replace YearInher_3_`year' =. if F5525==9998 | F5525==9999

gen MonthInher_3_`year'    =F5524
replace MonthInher_3_`year'=. if F5524==98 | F5524==99

gen InherAmt_3_`year'      =F5526
replace InherAmt_3_`year'  =. if F5526==9999998 | F5526==9999999

gen InherRecWho_3_`year'     =F5528


/*********************************************************************************************
	MERGE WITH RAND DATA
*********************************************************************************************/
keep h4hhid HHID SUBHH IRA1PercStocks IRA2PercStocks *Inher*
isid h4hhid
save "$CleanData/IRAStocks`year'.dta", replace
merge 1:m h4hhid using "$CleanData/temp1`year'.dta"
...
gen IRAStockWealth_NoImpute_`year'=IRA1StockWealth_NoImpute_`year' + IRA2StockWealth_NoImpute_`year'
...
gen IRAStockWealth_WithImpute_`year'=IRA1StockWealth_WithImpute_`year' + IRA2StockWealth_WithImpute_`year'
